...
data:
  ...
  bundledCode: "#line 1 \"verify/algorithm/LC_static_range_count_distinct.test.cpp\"\
    ...
    \ add_left, const F& add_right, const F& del_left, const F& del_right, const F&\
    \ query) {\n    const int q = left.size();\n    if (q == 0) return;\n    const\
    \ int width = max(1, (int)(n / sqrt(q)));\n    vector<int> order(q);\n    iota(order.begin(),\
  ...
  verifiedWith: []
documentation_of: verify/algorithm/LC_static_range_count_distinct.test.cpp
layout: document
redirect_from:
- /verify/verify/algorithm/LC_static_range_count_distinct.test.cpp
- /verify/verify/algorithm/LC_static_range_count_distinct.test.cpp.html
