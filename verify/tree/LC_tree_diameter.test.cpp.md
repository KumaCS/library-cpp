...
data:
  ...
  bundledCode: "#line 1 \"verify/tree/LC_tree_diameter.test.cpp\"\n#define PROBLEM\
    ...
    \ \";\n  _show(i + 1, a, c...);\n}\n#line 2 \"tree/tree-diameter.hpp\"\n\ntemplate\
    \ <class T>\npair<T, vector<int>> TreeDiameter(const vector<vector<pair<int, T>>>&\
    ...
    \      st.push({y, x});\n    }\n  }\n  int v = 0;\n  for (int i = 1; i < n; i++)\n\
    \    if (d[i] > d[v]) v = i;\n  vector<int> path;\n  for (int x = v; x != -1;\
    \ x = prv[x]) path.push_back(x);\n  return {d[v], path};\n}\npair<int, vector<int>>\
    \ TreeDiameter(const vector<vector<int>>& g) {\n  int n = g.size();\n  assert(n\
    \ > 0);\n  stack<pair<int, int>> st;\n  vector<int> d(n);\n  st.push({0, -1});\n\
    \  while (!st.empty()) {\n    auto [x, p] = st.top();\n    st.pop();\n    for\
    \ (auto y : g[x]) {\n      if (y == p) continue;\n      d[y] = d[x] + 1;\n   \
    \   st.push({y, x});\n    }\n  }\n  int u = 0;\n  for (int i = 1; i < n; i++)\n\
  ...
  verifiedWith: []
documentation_of: verify/tree/LC_tree_diameter.test.cpp
layout: document
redirect_from:
- /verify/verify/tree/LC_tree_diameter.test.cpp
- /verify/verify/tree/LC_tree_diameter.test.cpp.html
