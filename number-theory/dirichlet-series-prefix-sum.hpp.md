...
data:
  ...
  bundledCode: "#line 2 \"number-theory/dirichlet-series-prefix-sum.hpp\"\n\n// https://maspypy.com/dirichlet-%E7%A9%8D%E3%81%A8%E3%80%81%E6%95%B0%E8%AB%96%E9%96%A2%E6%95%B0%E3%81%AE%E7%B4%AF%E7%A9%8D%E5%92%8C\n\
    ...
    \    DP z(n);\n    return z.add(1, 1);\n  }\n\n  // zeta(s-k)\n  static DP zeta(u64\
    \ n, size_t k = 0) {\n    assert(k <= 3);\n    DP z(n);\n    for (size_t i = 1;\
    \ i <= z.K; i++) z.a[i] = mint((int)i).pow(k);\n    for (size_t i = 1; i <= z.K;\
    \ i++) z.s[i] = z.s[i - 1] + z.a[i];\n    for (size_t i = 1; i <= z.L; i++) {\n\
    \      u64 x = n / i;\n      if (k == 0)\n        z.S[i] = x;\n      else if (k\
    \ == 1)\n        z.S[i] = mint(x | 1) * mint((x + 1) / 2);\n      else if (k ==\
    \ 2) {\n        array<u64, 3> xs{x, x + 1, 2 * x + 1};\n        xs[x & 1] /= 2;\n\
    \        xs[(3 - (x % 3)) % 3] /= 3;\n        z.S[i] = mint(xs[0]) * mint(xs[1])\
    \ * mint(xs[2]);\n      } else if (k == 3) {\n        z.S[i] = mint(x | 1) * mint((x\
    \ + 1) / 2);\n        z.S[i] *= z.S[i];\n      }\n    }\n    return z;\n  }\n\n\
    \  // n(n+1)...(n+k-1)/k!\n  static DP rising(u64 n, size_t k) {\n    if (k ==\
    \ 0) return zeta(n);\n    DP z(n);\n    for (size_t i = 1; i <= z.K; i++) z.a[i]\
    \ = rising_mint(i, k);\n    for (size_t i = 1; i <= z.K; i++) z.s[i] = z.s[i -\
    \ 1] + z.a[i];\n    for (size_t i = 1; i <= z.L; i++) {\n      u64 x = n / i;\n\
    ...
    \ (size_t i = 1; i <= z.L; i++) {\n      u64 x = n / i;\n      z.S[i] = falling_mint(x\
    \ + 1, k + 1);  // -falling_mint(1, k + 1);\n    }\n    return z;\n  }\n\n  inline\
    ...
    \ l.a[x] * (t <= K ? r.s[t] : r.S[i * x]);\n      }\n      for (size_t y = 1;\
    \ y <= m; y++) {\n        u64 t = N / i / y;\n        v += r.a[y] * ((t <= K ?\
    \ l.s[t] : l.S[i * y]) - l.s[m]);\n      }\n      z.S[i] = v;\n    }\n    for\
    \ (size_t i = K; i >= 1; i--) {\n      for (size_t j = 2; u64(i) * j <= K; j++)\
    \ z.a[i * j] += l.a[i] * r.a[j];\n      z.a[i] *= r.a[1];\n    }\n    z.calc_small_sum();\n\
    \    return z;\n  }\n  friend DP operator/(const DP& l, const DP& r) {\n    DP\
    \ z(l);\n    const u64 N = z.N;\n    const int K = z.K;\n    const int L = z.L;\n\
    \    mint inv = r.a[1].inv();\n    for (size_t i = 1; i <= K; i++) {\n      z.a[i]\
    \ *= inv;\n      for (size_t j = 2; u64(i) * j <= K; j++) z.a[i * j] -= z.a[i]\
    \ * r.a[j];\n      z.s[i] = z.s[i - 1] + z.a[i];\n    }\n    for (size_t i = L;\
    \ i >= 1; i--) {\n      size_t m = sqrt(N / i);\n      for (size_t x = 1; x <=\
    \ m; x++) {\n        u64 t = N / i / x;\n        z.S[i] -= z.a[x] * (t <= K ?\
    \ r.s[t] : r.S[i * x]);\n      }\n      for (size_t y = 2; y <= m; y++) {\n  \
    \      u64 t = N / i / y;\n        z.S[i] -= r.a[y] * ((t <= K ? z.s[t] : z.S[i\
    \ * y]) - z.s[m]);\n      }\n      z.S[i] *= inv;\n      z.S[i] += z.s[m];\n \
    \   }\n    return z;\n  }\n  DP& operator*=(const DP& r) {\n    *this = DP(*this)\
    \ * r;\n    return *this;\n  }\n  DP& operator/=(const DP& r) {\n    *this = DP(*this)\
