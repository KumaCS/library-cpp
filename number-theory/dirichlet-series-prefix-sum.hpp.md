...
data:
  ...
  bundledCode: "#line 2 \"number-theory/dirichlet-series-prefix-sum.hpp\"\n\n// https://maspypy.com/dirichlet-%E7%A9%8D%E3%81%A8%E3%80%81%E6%95%B0%E8%AB%96%E9%96%A2%E6%95%B0%E3%81%AE%E7%B4%AF%E7%A9%8D%E5%92%8C\n\
    ...
    \ mint& r) const { return DP(*this) *= r; }\n\n  friend DP operator*(const DP&\
    \ l, const DP& r) {\n    DP z(l);\n    const int N = z.N;\n    const int K = z.K;\n\
    \    const int L = z.L;\n    for (size_t i = 1; i <= L; i++) {\n      mint v =\
    \ 0;\n      size_t m = sqrt(N / i);\n      for (size_t x = 1; x <= m; x++) {\n\
    \        u64 t = N / i / x;\n        v += l.a[x] * (t <= K ? r.s[t] : r.S[i *\
    ...
    \ j = 2; u64(i) * j <= K; j++) z.a[i * j] -= z.a[i] * r.a[j];\n      z.s[i] =\
    ...
    \  DP f(N);\n    for (auto [x, v] : r) {\n      for (size_t i = 1; i <= K / x;\
    \ i++) f.a[i * x] += v * a[i];\n      for (size_t i = 1; i <= L; i++) {\n    \
    \    u64 t = N / i / x;\n        f.S[i] += v * (t <= K ? s[t] : f.S[i * x]);\n\
    \      }\n    }\n    f.calc_small_sum();\n    return f;\n  }\n\n  friend ostream&\
    \ operator<<(ostream& os, const DP& f) {\n    os << \"a: \";\n    for (size_t\
    \ i = 1; i <= f.K; i++) os << f.a[i].val() << \",\\n\"[i == f.K];\n    os << \"\
